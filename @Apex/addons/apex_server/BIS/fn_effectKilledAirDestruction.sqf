/*
File: fn_effectKilledAirDestruction.sqf
Author:
...
Description:
	...
	BIS_fnc_effectKilledAirDestruction overwrite
...
if (isServer) then {
	...
	if ((_v isKindOf "Helicopter") && (getNumber (configfile >> "CfgVehicles" >> typeOf _v >> "isUav") == 1)) then {
		...
		_timeout = time + 30;
		waitUntil {(((getPos _v) select 2) < 0.3) || (time > _timeout)};	
		_pos = getpos _v;
		...
		_timeout = time + 30;
		waitUntil {(((getPos _v) select 2) < 2) || (time > _timeout)};
		_pos = getpos _v;
		if (!surfaceiswater(_pos)) then
		{	
			if ((speed _v) < 50) then {
