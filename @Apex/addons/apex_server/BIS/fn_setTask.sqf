...
_ids = _this param [0,"",["",[]]]; if (_ids isEqualType '') then {_ids = [_ids]};
_id = _ids param [0,"",[""]]; if (_id isEqualTo '') exitWith {["[x] Task ID cannot be an empty string!"] call bis_fnc_error;""};
_taskVar = _id call (missionNamespace getVariable 'BIS_fnc_taskVar');

//store previous data & check if the task is created (called for the very first time)
_prevData = [];
for '_index' from 0 to 11 step 1 do {_prevData pushBack (GET_DATA(_taskVar,_index));};
...
_taskIndex = (missionNamespace getVariable 'QS_mission_tasks') findIf {((_x select 0) isEqualTo (_this select 0))};
if (_taskIndex isEqualTo -1) then {
	(missionNamespace getVariable 'QS_mission_tasks') pushBack [(_this select 0),[(_this select 8),'',''],[FALSE,-1],[FALSE,-1]];
};
...
_fnc_addOwner = {
	...
	if ((_this isEqualType grpNull) || {(_this isEqualType '')} || {(_this isEqualType sideUnknown)}) exitWith {
		_owners pushBackUnique _this;
	};
	if (_this isEqualType objNull) exitWith {
		_thisVar = _this call (missionNamespace getVariable 'BIS_fnc_objectVar');
		_owners pushBackUnique _thisVar;	
	};
...
if (_destTarget isEqualType objNull) then
{
	_dest =
	[
		_destTarget,
...
--------------------------------------------------------------------------------------------------*/
private _global = _this param [7,true,[true]]; 
if !(isMultiplayer) then {_global = false;};
...
if (_txt isEqualType '') then
{
	...
	private _cfgTaskDescription = if (_txt isEqualTo '') then
	{
...

{
	...
	if (!(_text isEqualType [])) then {_text = [_text]};
...
if (_state isEqualType TRUE) then {_state = toupper([GET_DATA(_taskVar,STATE),"ASSIGNED"] select _state)};
...
if (!(_parent isEqualTo '')) then
{
	...
	_added = false;
	if ((_parentChildren findIf {(_x == _id)}) isEqualTo -1) then
	{
